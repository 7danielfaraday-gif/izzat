...
export async function onRequest(context) {
  try {
    if (context.env && context.env.ASSETS && typeof context.env.ASSETS.fetch === 'function') {
      return context.env.ASSETS.fetch(buildAssetRequest(context.request, '/c/'));
    }
  } catch (error) {
    console.error('[routes:c] asset fetch failed', error);
  }

  const url = new URL(context.request.url);
  return Response.redirect(`${url.origin}/c/?product=${encodeURIComponent(context.params.slug || '')}`, 302);
}
