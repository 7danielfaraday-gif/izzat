...
function safePickBody(body) {
  // Remove anything that looks like card/password/token and clamp sizes.
  const denyKey = /(card|cvv|senha|password|passwd|token|secret|pix_code|pixcode|qrcode)/i;

  const out = {};
  if (!isObject(body)) return out;

  for (const [k0, v] of Object.entries(body)) {
    const k = String(k0 || '').slice(0, 60);
    if (!k || denyKey.test(k)) continue;

    if (typeof v === 'string') {
      const s = clampStr(v, 1200);
    ...
    if (Array.isArray(v)) {
      ...
      const arr = v
        .slice(0, 30)
        .map((x) => (typeof x === 'string' ? clampStr(x, 200) : typeof x === 'number' || typeof x === 'boolean' ? x : null))
    ...
    if (isObject(v)) {
      // One level deep
      const obj = {};
      for (const [kk0, vv] of Object.entries(v)) {
        const kk = String(kk0 || '').slice(0, 60);
        if (!kk || denyKey.test(kk)) continue;
        if (typeof vv === 'string') {
          const s = clampStr(vv, 500);
          if (s !== undefined) obj[kk] = s;
...
export async function onRequestPost(context) {
  try {
    ...
    payload.ip = clampStr(context.request.headers.get('cf-connecting-ip') || '', 64);
    payload.ua = clampStr(context.request.headers.get('user-agent') || '', 220);
    payload.cf_ray = clampStr(context.request.headers.get('cf-ray') || '', 64);
