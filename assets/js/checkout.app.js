document.addEventListener('DOMContentLoaded', function(){
 ...
 if (typeof window.getProductConfig === 'function' && !window.__PRODUCT_CONFIG_READY) {
 window.getProductConfig().finally(function() {
 setTimeout(window.initReactCheckout, 0);
 });
 return;
 }
 ...
 const ACTIVE_PRODUCT = window.__PRODUCT_CONFIG || window.__PRODUCT_DEFAULT || {};
 const PRODUCT_INFO = { 
 ...
 const CHECKOUT_TEXT = PRODUCT_INFO.checkout || {};
...
body: JSON.stringify({ id: uniqueOrderId, name: formData.name, email: finalEmail, phone: finalPhone, cpf: formData.cpf || '', cep: formData.cep || '', address: formData.address || '', number: formData.number || '', city: formData.city || '', product_slug: PRODUCT_INFO.slug || '', product_id: PRODUCT_INFO.id || '', product_name: PRODUCT_INFO.name || '', value: PRODUCT_INFO.price })
 ...
 window.location.href = window.__PRODUCT_LANDING_PATH || '/';
 }
 } catch(e) { window.location.href = window.__PRODUCT_LANDING_PATH || '/'; }
...
e("img", { src: "/assets/img/logo.webp", alt: "Logo", className: "h-8 w-auto object-contain", onError: (ev) => { try { const img = ev.target; if(!img.dataset.fallback){ img.dataset.fallback='1'; img.src = "/assets/img/logo.webp"; } } catch(e) {} } }),
 ...
 e("div", null, e("p", { className: "font-bold text-amber-900 text-sm mb-0.5" }, CHECKOUT_TEXT.guarantee_title || "Satisfação Garantida"), e("p", { className: "text-amber-800/80 text-xs leading-relaxed" }, CHECKOUT_TEXT.guarantee_text || "Se não gostar, devolvemos seu dinheiro em até 7 dias. Sem burocracia."))
 )
 )
 ),
...
e("span", {className: "text-[9px] text-slate-600 bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200 normal-case"}, "Opcional")
 ...
 e("div", { className: "absolute -top-3 right-4 bg-gradient-to-r from-green-600 to-green-500 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase shadow-md tracking-wide" }, CHECKOUT_TEXT.payment_badge || "Aprovação Imediata"),
 ...
 e("div", {className: "flex-1 py-1"}, e("div", { className: "font-bold text-slate-800 text-sm flex items-center gap-1.5" }, CHECKOUT_TEXT.payment_label || "PIX"), e("div", { className: "text-green-700 font-extrabold text-xl mt-0.5 tracking-tight" }, "R$ " + PRODUCT_INFO.price.toFixed(2).replace('.',',')))
 ),
 ...
 loading ? e("span", {className: "flex items-center gap-2"}, e("div", { className: "spinner-mobile" }), "Processando...") : e("span", {className: "flex items-center gap-2"}, CHECKOUT_TEXT.submit_label || "FINALIZAR COM DESCONTO", e("svg", { className: "w-5 h-5", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "3" }, e("polyline", {points: "9 18 15 12 9 6"})))
 ),
 )
 )
 ),
...
e("p", {className: "text-[10px] text-slate-300 leading-relaxed"}, CHECKOUT_TEXT.footer_primary || "Compra processada por Izzat \u00A9 2026"),
e("p", {className: "text-[10px] text-slate-300 mt-0.5"}, CHECKOUT_TEXT.footer_secondary || "Todos os direitos reservados")
)
),
...
trackEvent('CompletePayment', { ...window.PRODUCT_CONTENT, content_name: PRODUCT_INFO.name, value: PRODUCT_INFO.price, currency: (window.PRODUCT_CONTENT && window.PRODUCT_CONTENT.currency) || 'BRL', order_id: customerData.transactionId, event_id: window.generateEventId(), email: customerData.email, phone: customerData.phone });
}
...
e("h1", {className: "text-lg font-extrabold text-slate-800 mb-0.5"}, (CHECKOUT_TEXT.success_title_template || "Quase lá, {firstName}!").replace("{firstName}", firstName)),
e("p", {className: "text-xs text-slate-400"}, CHECKOUT_TEXT.success_subtitle || "Finalize o pagamento para garantir a oferta.")
),
...
((CHECKOUT_TEXT.payment_instructions && CHECKOUT_TEXT.payment_instructions.length) ? CHECKOUT_TEXT.payment_instructions : [
...
e("p", {className: "text-center text-[11px] text-slate-300 font-mono mt-3"}, "ID: " + transactionId)
 ...
 (async () => {
 try {
 const slugParam = PRODUCT_INFO.slug ? `slug=${encodeURIComponent(PRODUCT_INFO.slug)}&` : '';
 const res = await fetch(`/api/pix-config?${slugParam}_=${Date.now()}`, { cache: 'no-store' });
